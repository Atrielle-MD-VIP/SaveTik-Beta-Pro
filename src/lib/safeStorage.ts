...
export const safeLocalStorage = {
  getItem: (key: string, fallback: string | null = null): string | null => {
    try {
      if (typeof window !== 'undefined' && window.localStorage) {
        return window.localStorage.getItem(key) ?? memoryStorage[key] ?? fallback;
      }
    } catch (e) {
      console.warn(`[SaveTik] localStorage.getItem failed for key "${key}", using memory storage:`, e);
    }
  ...
  setItem: (key: string, value: string): boolean => {
    try {
      ...
      console.warn(`[SaveTik] localStorage.setItem failed for key "${key}", saved to memory storage:`, e);
      return true;
    }
    return true;
  },
