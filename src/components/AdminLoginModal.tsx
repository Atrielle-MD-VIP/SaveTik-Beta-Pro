...
interface AdminLoginModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (sessionId?: string, remember?: boolean) => void;
}
    ...
    try {
      ...
      if (response.ok && data.success) {
        if (data.requireToken) {
          ...
          setError('');
          onSuccess(data.sessionId, rememberLogin);
        }
      } else {
            ...
            <>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-200">
                <label className="block text-xs font-bold uppercase tracking-wide text-slate-300 mb-1.5 flex justify-between items-center">
                  <span>{is2fa ? 'Masukin Kode 2FA' : 'Masukin Token Akses'}</span>
                  <span className="text-[10px] text-emerald-400 capitalize normal-case font-normal animate-pulse">{is2fa ? 'Authenticator' : 'Token Akses'}</span>
                </label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder={is2fa ? "Contoh: 123456" : "Masukkan Token"}
                    required
                    autoFocus
                  />
                  <button
