...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const [cooldownMins, setCooldownMins] = useState<number>(() => {
    const secValStr = safeLocalStorage.getItem('savetik-cooldown-seconds');
    if (secValStr !== null) {
      return Math.floor(parseInt(secValStr, 10) / 60);
    }
    const val = safeLocalStorage.getItem('savetik-cooldown-minutes');
  ...
  const [cooldownSecs, setCooldownSecs] = useState<number>(() => {
    const secValStr = safeLocalStorage.getItem('savetik-cooldown-seconds');
    if (secValStr !== null) {
      return parseInt(secValStr, 10) % 60;
    }
    return 0;
  });
  ...
  const [isConfirmingResetLive, setIsConfirmingResetLive] = useState<boolean>(false);
  ...
  const adminFetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    const localSessionId = safeLocalStorage.getItem('savetik-admin-session-id');
    const sessionSessionId = typeof window !== 'undefined' && window.sessionStorage ? window.sessionStorage.getItem('savetik-admin-session-id') : null;
    const sessionId = localSessionId || sessionSessionId || '';
  ...
  const handleUpdateCredentials = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        if (data.newToken) {
          safeLocalStorage.setItem('savetik-admin-token', data.newToken);
        }
        alert('Kredensial admin & token berhasil diperbarui! Silakan login kembali.');
  ...
  const handleClearPlatformStats = async () => {
    try {
      const res = await adminFetch('/api/admin/platform-stats', { method: 'DELETE' });
      const text = await res.text();
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        alert('Gagal menghapus statistik platform: Server merespons dengan HTML.');
        return;
      }
      const data = JSON.parse(text);
      if (data.success) {
        setPlatformStats([
          { name: 'TikTok', percentage: 0, count: 0, color: '#00F2FE', bgClass: 'bg-cyan-500' },
          { name: 'YouTube', percentage: 0, count: 0, color: '#FF0000', bgClass: 'bg-red-600' },
          { name: 'Instagram', percentage: 0, count: 0, color: '#E1306C', bgClass: 'bg-pink-600' },
          { name: 'CapCut', percentage: 0, count: 0, color: '#F59E0B', bgClass: 'bg-amber-500' },
          { name: 'Spotify', percentage: 0, count: 0, color: '#1DB954', bgClass: 'bg-emerald-500' },
          { name: 'Facebook', percentage: 0, count: 0, color: '#1877F2', bgClass: 'bg-blue-600' }
        ]);
        alert('Statistik sumber platform berhasil dibersihkan!');
      } else {
        alert('Gagal membersihkan statistik: ' + data.message);
      }
    } catch (err) {
  ...
  const handleClearDemographics = async () => {
    try {
      const res = await adminFetch('/api/admin/demographics', { method: 'DELETE' });
      const text = await res.text();
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        alert('Gagal menghapus demografi: Server merespons dengan HTML.');
        return;
      }
      const data = JSON.parse(text);
      if (data.success) {
        setDemographics([]);
        safeLocalStorage.setItem('savetik-demographics', JSON.stringify({ regions: {}, total: 0 }));
        alert('Data demografi wilayah berhasil dibersihkan!');
      } else {
        alert('Gagal membersihkan demografi: ' + data.message);
      }
    } catch (err) {
      alert('Terjadi kesalahan saat menghapus demografi.');
    }
  };

  const handleClearTotalDownloads = async () => {
    try {
      const res = await adminFetch('/api/admin/total-downloads', { method: 'DELETE' });
      const text = await res.text();
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        alert('Gagal mereset total unduhan: Server merespons dengan HTML.');
        return;
      }
      const data = JSON.parse(text);
      if (data.success) {
        setStats(prev => ({ ...prev, totalDownloads: 0 }));
        alert('Total unduhan berhasil direset!');
      } else {
        alert('Gagal mereset: ' + data.message);
      }
    } catch (err) {
      alert('Terjadi kesalahan saat mereset total unduhan.');
    }
  };

  const handleClearActiveUsers = async () => {
    try {
      const res = await adminFetch('/api/admin/active-users', { method: 'DELETE' });
      const text = await res.text();
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        alert('Gagal mereset pengguna aktif: Server merespons dengan HTML.');
        return;
      }
      const data = JSON.parse(text);
      if (data.success) {
        setStats(prev => ({ ...prev, activeUsersNow: 0, activeUsersToday: 0 }));
        alert('Data pengguna aktif berhasil direset!');
      } else {
        alert('Gagal mereset: ' + data.message);
      }
    } catch (err) {
      alert('Terjadi kesalahan saat mereset pengguna aktif.');
    }
  };

  const handleClearDonations = async () => {
    try {
      const res = await adminFetch('/api/admin/donations', { method: 'DELETE' });
      const text = await res.text();
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        alert('Gagal mereset donasi: Server merespons dengan HTML.');
        return;
      }
      const data = JSON.parse(text);
      if (data.success) {
        setStats(prev => ({ ...prev, donationsToday: 0, donatorsCount: 0 }));
        alert('Data donasi hari ini berhasil direset!');
      } else {
        alert('Gagal mereset: ' + data.message);
      }
    } catch (err) {
      alert('Terjadi kesalahan saat mereset donasi.');
    }
  };

  const handleResetLiveStats = async () => {
    if (!isConfirmingResetLive) {
      setIsConfirmingResetLive(true);
      setTimeout(() => {
        setIsConfirmingResetLive(false);
      }, 4000); // Reset confirmation state after 4 seconds
      return;
    }
    try {
      const res = await adminFetch('/api/admin/reset-live-stats', { method: 'DELETE' });
      const text = await res.text();
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        alert('Gagal mereset statistik live: Server merespons dengan HTML.');
        setIsConfirmingResetLive(false);
        return;
      }
      const data = JSON.parse(text);
      if (data.success) {
        ...
        alert('Statistik widget live real-time berhasil direset menjadi 0!');
      } else {
        alert('Gagal mereset statistik live: ' + data.message);
      }
    } catch (err) {
      alert('Terjadi kesalahan saat mereset statistik live.');
    } finally {
      setIsConfirmingResetLive(false);
    }
  };
  ...
  const [faqsActive, setFaqsActive] = useState(true);
  const [faqsSubheading, setFaqsSubheading] = useState('Jawaban lengkap seputar mekanisme verifikasi dan lisensi Alight Motion Pro di AlightPro.');
          ...
          if (data.health) {
            ...
            if (data.analytics) {
              ...
              setStats(prev => ({
                ...
                liveTotalDownloads: ana.liveTotalDownloads !== undefined ? ana.liveTotalDownloads : 0,
                liveDownloadsToday: ana.liveDownloadsToday !== undefined ? ana.liveDownloadsToday : 0
              }));
            ...
            const combinedRegions: Record<string, number> = {};
            const demoData = safeLocalStorage.getJSON<any>('savetik-demographics', null);
            if (demoData && demoData.regions) {
              for (const [k, v] of Object.entries(demoData.regions)) {
                const count = typeof v === 'object' && v !== null ? ((v as any).count || 1) : (Number(v) || 1);
                combinedRegions[k] = (combinedRegions[k] || 0) + count;
              }
            }
            if (ana.regions) {
              for (const [k, v] of Object.entries(ana.regions)) {
                const count = Number(v) || 1;
                combinedRegions[k] = Math.max(combinedRegions[k] || 0, count);
              }
            }

            const total = Object.values(combinedRegions).reduce((a: any, b: any) => a + b, 0) as number || 1;
            const dArr = Object.entries(combinedRegions).map(([region, count]: [string, any]) => {
              let countryName = region;
              if (region.includes('Kuala_Lumpur')) countryName = 'Malaysia 🇲🇾';
              else if (region.includes('Singapore')) countryName = 'Singapore 🇸🇬';
              else if (region.includes('Jakarta') || region.includes('Makassar') || region.includes('Jayapura') || region.includes('Indonesia') || region.includes('Asia/Jakarta')) countryName = 'Indonesia 🇮🇩';
              else if (region.includes('Bangkok')) countryName = 'Thailand 🇹🇭';
              else if (!region.includes('/')) countryName = region;
              else countryName = `Wilayah (${region})`;

              return {
                country: countryName,
                cities: `Zona Waktu: ${region}`,
                percentage: Math.round((count / total) * 100) || 100,
  ...
  useEffect(() => {
    fetchDashboardData();
    const interval = setInterval(fetchDashboardData, 3000); // Refresh every 3s for real-time
    return () => clearInterval(interval);
  }, []);
  ...
  useEffect(() => {
    ...
    // Async fetch from server to sync
  ...
  const handleSaveFaqSettings = async (activeVal: boolean, subheadingVal: string) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        if (typeof data.active === 'boolean') setFaqsActive(data.active);
        if (typeof data.subheading === 'string') setFaqsSubheading(data.subheading);
        safeLocalStorage.setItem('savetik-faqs-active', String(data.active));
        if (data.subheading) safeLocalStorage.setItem('savetik-faqs-subheading', data.subheading);
        alert('Pengaturan global FAQ berhasil diperbarui!');
        window.dispatchEvent(new Event('savetik-faqs-changed'));
      }
    } catch (err) {
      console.error('Error saving FAQ settings:', err);
      alert('Gagal menyimpan pengaturan FAQ.');
    }
  };
  ...
  const handleAddFaq = async (e: React.FormEvent) => {
    ...
    try {
      const res = await adminFetch('/api/admin/faqs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ faqs: newFaqList, active: faqsActive, subheading: faqsSubheading })
      });
      const data = await res.json();
      if (data.success) {
        if (Array.isArray(data.faqs)) {
      ...
      console.error('Error adding FAQ to server:', err);
    }
  };
  ...
  const handleDeleteFaq = async (index: number) => {
    ...
    try {
      ...
      console.error('Error deleting FAQ from server:', err);
    }
  };

  const handleUpdateFaq = async (index: number) => {
    if (!editingFaqQ.trim() || !editingFaqA.trim()) return;
    const newFaqList = faqs.map((faq, i) => {
      if (i === index) {
        return { question: editingFaqQ.trim(), answer: editingFaqA.trim() };
      }
      return faq;
    });
    ...
    try {
      ...
      console.error('Error updating FAQ on server:', err);
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300 pb-20 px-2 sm:px-4">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
            ...
            <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-2xl shadow-xl text-white relative overflow-hidden backdrop-blur-sm group hover:border-indigo-500/50 transition-all">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={handleClearTotalDownloads}
                    className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 rounded-lg border border-rose-500/20 transition-all text-xs flex items-center gap-1 cursor-pointer z-20 relative"
                    title="Reset Total Unduhan"
                  >
                    <Trash2 size={13} />
            ...
            <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-2xl shadow-xl text-white relative overflow-hidden backdrop-blur-sm group hover:border-amber-500/50 transition-all">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={handleClearActiveUsers}
                    className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 rounded-lg border border-rose-500/20 transition-all text-xs flex items-center gap-1 cursor-pointer z-20 relative"
                    title="Reset Pengguna Aktif"
                  >
                    <Trash2 size={13} />
                    <span className="text-[10px]">Reset</span>
                  </button>
          ...
          {/* Kontrol Widget Live Real-Time */}
          <div className="bg-slate-900/80 border border-slate-800 p-4 rounded-xl shadow-lg text-white backdrop-blur-sm hover:border-yellow-500/50 transition-all">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-3 mb-3">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-yellow-500/20 border border-yellow-500/30 rounded-lg text-yellow-300">
                  <Activity size={16} />
                </div>
                <div>
                  <h3 className="font-heading font-black uppercase text-xs tracking-wide text-white">
                    Statistik Widget Real-Time (SaveTik Live)
                  </h3>
                  <p className="text-[10px] text-slate-400">
                    Statistik real-time yang ditampilkan pada widget halaman utama
                  </p>
                </div>
              </div>
              <button
                ...
                className={`py-0.5 px-1.5 font-bold rounded-md border transition-all text-[8px] flex items-center gap-1 cursor-pointer ${
                  isConfirmingResetLive 
                    ? "bg-amber-500 hover:bg-amber-600 border-amber-600 text-white animate-pulse shadow-[1px_1px_0px_0px_rgba(245,158,11,0.3)]" 
                    : "bg-rose-500 hover:bg-rose-600 border-rose-600 text-white shadow-[1px_1px_0px_0px_rgba(244,63,94,0.3)]"
                } hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5`}
                title="Reset Statistik Live Widget"
              >
                <Trash2 size={9} />
                <span>{isConfirmingResetLive ? "Yakin?" : "Reset ke 0"}</span>
              </button>
            </div>

            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-800">
                <span className="text-[10px] text-slate-400 block">Hari Ini</span>
                <span className="text-base font-black font-mono text-yellow-400">
                  {(stats.liveDownloadsToday || 0).toLocaleString('id-ID')}
                </span>
              </div>
              <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-800">
                <span className="text-[10px] text-slate-400 block">Total Unduhan</span>
                <span className="text-base font-black font-mono text-cyan-400">
                  {(stats.liveTotalDownloads || 0).toLocaleString('id-ID')}
                </span>
              </div>
              <div className="bg-slate-950/60 p-2 rounded-lg border border-slate-800">
                <span className="text-[10px] text-slate-400 block">Online</span>
                <span className="text-base font-black font-mono text-emerald-400">
                  {(stats.activeUsersNow || 0).toLocaleString('id-ID')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl text-white space-y-5 backdrop-blur-sm">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs font-medium text-slate-400 hidden sm:inline">Distribusi Media</span>
                  <button
                    onClick={handleClearPlatformStats}
                    className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 rounded-lg border border-rose-500/20 transition-all text-xs flex items-center gap-1 cursor-pointer"
                    title="Reset Statistik Platform"
                  >
                    <Trash2 size={14} />
                    <span>Reset</span>
                  </button>
            ...
            <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl text-white space-y-5 backdrop-blur-sm">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold uppercase text-emerald-300 bg-emerald-500/20 border border-emerald-500/30 px-2 py-0.5 rounded-full hidden sm:inline">
                    TELEMETRI NYATA
                  </span>
                  <button
                    onClick={handleClearDemographics}
                    className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 rounded-lg border border-rose-500/20 transition-all text-xs flex items-center gap-1 cursor-pointer"
                    title="Reset Demografi Wilayah"
                  >
                    <Trash2 size={14} />
                    <span>Reset</span>
                  </button>
      ...
      {activeTab === 'faqs' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
          <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl text-white backdrop-blur-sm">
            ...
            {/* FAQ Global Settings */}
            <div className="mb-6 p-5 bg-slate-950/70 border border-slate-800 rounded-2xl space-y-4">
              <h4 className="font-heading font-black text-sm uppercase text-white flex items-center gap-2">
                <Wrench size={16} className="text-blue-400" />
                Pengaturan Global Tampilan FAQ
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                <div className="flex items-center justify-between p-3 bg-slate-900 border border-slate-800 rounded-xl">
                  <div>
                    <span className="block text-xs font-bold uppercase text-white">Status Tampilan FAQ</span>
                    <span className="text-[10px] text-slate-400 font-medium">Sembunyikan atau aktifkan FAQ di landing</span>
                  </div>
                  <button
                    onClick={() => {
                      const nextVal = !faqsActive;
                      setFaqsActive(nextVal);
                      handleSaveFaqSettings(nextVal, faqsSubheading);
                    }}
                    type="button"
                    className={`px-4 py-2 text-xs font-black uppercase tracking-wider rounded-lg transition-all border-2 cursor-pointer ${
                      faqsActive 
                        ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' 
                        : 'bg-rose-500/20 text-rose-400 border-rose-500/50'
                    }`}
                  >
                    {faqsActive ? 'MUNCUL' : 'DISEMBUNYIKAN'}
                  </button>
                </div>

                <div>
                  <label className="block text-xs font-bold uppercase text-slate-300 mb-1">Subjudul FAQ</label>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      value={faqsSubheading}
                      onChange={(e) => setFaqsSubheading(e.target.value)}
                      placeholder="Masukkan subjudul FAQ..."
                      className="flex-grow bg-slate-900 border border-slate-700 p-2 text-xs font-bold text-white focus:outline-none focus:border-blue-500 rounded-lg"
                    />
                    <button
                      onClick={() => handleSaveFaqSettings(faqsActive, faqsSubheading)}
                      type="button"
                      className="px-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs uppercase rounded-lg transition-all cursor-pointer"
                    >
                      UPDATE
                    </button>
                  </div>
            ...
            <div className="space-y-3">
              ...
              {faqs.map((faq, index) => {
                ...
                return isEditing ? (
                  <div key={index} className="p-5 bg-slate-900 border-2 border-amber-500 rounded-xl space-y-4">
                    <div className="space-y-3">
                      <div>
                        <label className="block text-[10px] font-bold uppercase text-amber-400 mb-1">Edit Pertanyaan</label>
                        <input 
                          type="text" 
                          value={editingFaqQ}
                          onChange={(e) => setEditingFaqQ(e.target.value)}
                          className="w-full bg-slate-950 border border-slate-700 p-2.5 text-xs font-bold text-white focus:outline-none focus:border-amber-500 rounded-lg"
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase text-amber-400 mb-1">Edit Jawaban</label>
                        <textarea 
                          value={editingFaqA}
                          onChange={(e) => setEditingFaqA(e.target.value)}
                          rows={2}
                          className="w-full bg-slate-950 border border-slate-700 p-2.5 text-xs font-bold text-white focus:outline-none focus:border-amber-500 resize-none rounded-lg"
                        />
                      </div>
                    </div>
                    <div className="flex gap-2 justify-end">
                      <button 
                        ...
                        className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-[10px] uppercase rounded-lg transition-all flex items-center gap-1 cursor-pointer"
                      >
                        <X size={12} />
                        Batal
                      </button>
                      <button 
                        ...
                        className="px-3 py-1.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white font-bold text-[10px] uppercase rounded-lg transition-all flex items-center gap-1 cursor-pointer"
                      >
                        <Check size={12} />
                        Simpan Perubahan
                      </button>
                  ...
                  <div key={index} className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl flex justify-between items-start gap-4 hover:border-slate-700 transition-colors">
                    ...
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button 
                        ...
                        className="p-2 bg-rose-500/20 border border-rose-500/30 hover:bg-rose-500 text-rose-300 hover:text-white rounded-lg transition-all cursor-pointer"
                        title="Hapus FAQ ini"
                      >
                        <Trash2 size={14} />
                      </button>
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-2 text-white">
          ...
          <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl backdrop-blur-sm">
            ...
            <form onSubmit={handleUpdateCredentials} className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="block text-[10px] font-bold uppercase text-slate-400 ml-1">Token Akses / Secret Token Baru (Opsional)</label>
                <input
                  type="text"
                  value={newAdminToken}
                  onChange={(e) => setNewAdminToken(e.target.value)}
                  className="w-full bg-slate-950/50 border border-slate-800 p-3 text-sm text-white focus:outline-none focus:border-indigo-500 rounded-xl transition-all font-mono"
      ...
      {activeTab === 'apk-update' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2 text-white">
          ...
          {/* Cooldown Settings Mode */}
          <div className="bg-slate-900/80 border border-slate-800 p-6 rounded-2xl shadow-xl backdrop-blur-sm">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`p-2.5 border rounded-2xl text-white ${(cooldownMins > 0 || cooldownSecs > 0) ? "bg-indigo-500/20 border-indigo-500/40 text-indigo-300" : "bg-slate-850 border-slate-800 text-slate-500"}`}>
                  <Clock size={24} />
                </div>
                <div>
                  <h4 className="font-heading font-black text-sm uppercase text-white">
                    Batas Waktu Download
                  </h4>
                  <p className="text-xs text-slate-300 font-medium">
                    Jeda waktu antar download menit/detik Isi (0) untuk menonaktifkan.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isNaN(val) && val >= 0) {
                      setCooldownMins(val);
                      const totalSecs = (val * 60) + cooldownSecs;
                      safeLocalStorage.setItem('savetik-cooldown-seconds', totalSecs.toString());
                    }
                  }}
                  className="w-16 px-3 py-2 bg-slate-800/50 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 text-center"
                />
                <span className="text-slate-400 text-xs font-bold uppercase">Menit</span>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isNaN(val) && val >= 0) {
                      setCooldownSecs(val);
                      const totalSecs = (cooldownMins * 60) + val;
                      safeLocalStorage.setItem('savetik-cooldown-seconds', totalSecs.toString());
                    }
                  }}
                  className="w-16 px-3 py-2 bg-slate-800/50 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 text-center ml-2"
                />
                <span className="text-slate-400 text-xs font-bold uppercase">Detik</span>
              </div>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-800 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => {
                  alert(`Waktu cooldown disimpan: ${cooldownMins} menit ${cooldownSecs} detik.`);
                }}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold uppercase rounded-lg transition-colors border border-indigo-500 shadow-lg shadow-indigo-500/30 flex items-center gap-2 cursor-pointer"
              >
                <Save size={14} />
                Simpan
              </button>
              <button
                type="button"
                onClick={() => {
                  safeLocalStorage.removeItem('savetik-last-extract-time');
                  alert('Waktu tunggu download berhasil direset untuk sesi ini.');
                }}
                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold uppercase rounded-lg transition-colors border border-slate-700 flex items-center gap-2 cursor-pointer"
              >
                <RotateCw size={14} />
                Reset Sesi Saat Ini
              </button>
