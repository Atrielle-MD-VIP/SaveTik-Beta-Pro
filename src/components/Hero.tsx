...
import { Download, Clipboard, Link2, Sparkles, CheckCircle2, AlertTriangle, Clock } from 'lucide-react';
...
export default function Hero({ onExtract, loading, showToast, onAdminAccess }: HeroProps) {
  ...
  useEffect(() => {
    const checkCooldown = () => {
      const lastTimeStr = safeLocalStorage.getItem('savetik-last-extract-time');
      const cooldownSecStr = safeLocalStorage.getItem('savetik-cooldown-seconds');
      
      let cooldownMs = 0;
      if (cooldownSecStr !== null) {
        cooldownMs = parseInt(cooldownSecStr, 10) * 1000;
      } else {
        const cooldownMinStr = safeLocalStorage.getItem('savetik-cooldown-minutes') || '0';
        cooldownMs = parseFloat(cooldownMinStr) * 60 * 1000;
      }
      
      if (lastTimeStr && cooldownMs > 0) {
  ...
  return (
    <section className="pt-4 pb-1 md:pt-8 md:pb-2 px-4 md:px-8 max-w-5xl mx-auto text-center">
      {/* Badge Decorative */}
      <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
        <div className="inline-flex items-center gap-1.5 bg-[#DBEAFE] dark:bg-[#1E293B] text-[#1E3A8A] dark:text-blue-300 neo-border-thin px-3 py-1.5 rounded-[30px] text-xs font-black uppercase tracking-wider rotate-[-1deg] shadow-xs">
          <Sparkles 
            size={17} 
            className="text-[#1E3A8A] dark:text-blue-300 animate-sparkle-spin cursor-pointer hover:scale-110 active:scale-95 transition-transform" 
      ...
      {/* Fluid Main Heading */}
      <h1 className="font-heading font-black tracking-tighter uppercase leading-[0.9] text-neo-text mb-3 text-4xl sm:text-5xl md:text-6xl lg:text-7xl">
        SATU TAUTAN <br />
        <span className="bg-[#E11D48] text-white px-3 inline-block rotate-[1deg] neo-border my-1">
          SEMUA PLATFORM
        </span>
      </h1>

      <p className="max-w-2xl mx-auto text-neo-text opacity-70 font-medium text-base md:text-lg mb-5 leading-relaxed">
      ...
      <div className="max-w-3xl mx-auto mb-2">
        ...
        >
          <div className="flex-1 flex flex-col gap-3">
            ...
            {/* Cooldown Timer */}
            {cooldownRemaining > 0 && (
              <div className="flex items-center gap-3 px-1 mt-1 animate-in fade-in slide-in-from-top-1 duration-300">
                <div className="flex items-center gap-2 bg-[#FEF2F2] dark:bg-[#450a0a] border border-[#FCA5A5] dark:border-[#7f1d1d] px-3 py-1.5 rounded-lg w-full">
                   <Clock size={14} className="text-[#DC2626] dark:text-[#f87171]" />
                  <span className="text-[11px] font-bold text-[#DC2626] dark:text-[#f87171]">
                    Mohon tunggu waktu {Math.floor(cooldownRemaining / 60) > 0 ? `${Math.floor(cooldownRemaining / 60)} menit ` : ''}{cooldownRemaining % 60} detik untuk mendownload lagi
                  </span>
                </div>
              </div>
            )}
