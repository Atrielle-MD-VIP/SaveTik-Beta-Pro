...
// Global safety handlers to prevent process exit on background async rejections (e.g. gRPC auth / Railway network timeouts)
process.on('unhandledRejection', (reason, promise) => {
  console.warn('Unhandled Rejection caught (non-fatal):', reason);
});
...
// In otplib v13, the default authenticator instance is not exported in ESM.
// We create a wrapper that matches the expected API for the rest of the application.
const authenticator = {
  generateSecret: (len?: number) => otplib.generateSecret({ length: len }),
  keyuri: (label: string, issuer: string, secret: string) => 
    otplib.generateURI({ label, issuer, secret }),
  verify: (opts: { token: string; secret: string }) => {
    try {
      if (!opts.token || opts.token.length !== 6) return false;
      const res = otplib.verifySync(opts);
...
const initFirebase = async () => {
  try {
    ...
    const rawSa = process.env.FIREBASE_SERVICE_ACCOUNT || process.env.FIREBASE_SERVICE_ACCOUNT_KEY;
    if (rawSa) {
      try {
        const jsonStr = rawSa.trim().startsWith('{') 
          ? rawSa.trim() 
          : Buffer.from(rawSa.trim(), 'base64').toString('utf8');
    ...
    if (getApps().length === 0) {
      if (serviceAccount) {
        console.log('Firebase: Initializing Admin SDK with explicit Service Account credentials.');
        initializeApp({
    ...
    // Detect if running in non-GCP environment (e.g. Railway, Heroku, local) without Google Application Default Credentials
    const isGcpEnvironment = !!(process.env.K_SERVICE || process.env.GCLOUD_PROJECT || process.env.GOOGLE_CLOUD_PROJECT || process.env.GCP_PROJECT);
    const isRailway = !!(process.env.RAILWAY_SERVICE_ID || process.env.RAILWAY_ENVIRONMENT || process.env.RAILWAY_STATIC_URL);
    const hasExplicitAuth = !!(serviceAccount || process.env.GOOGLE_APPLICATION_CREDENTIALS);

    // If running on Railway or outside GCP without explicit Service Account, skip Firestore gRPC calls.
    // Calling getFirestore().get() without GCP ADC on Railway causes gRPC background timer crashes.
    if ((isRailway || !isGcpEnvironment) && !hasExplicitAuth) {
      console.warn('Firebase: Non-GCP environment (e.g. Railway) detected without FIREBASE_SERVICE_ACCOUNT credential.');
      console.warn('Firebase: Skipping Firestore ADC metadata check to prevent gRPC crashes. Running in local/in-memory mode.');
      return null;
    }

    // Helper for timeout to prevent hangs on metadata query
    ...
    // Prefer named database if provided, as it is likely the one provisioned
    if (databaseId && databaseId !== '(default)') {
      try {
        console.log(`Firebase: Attempting connection to named database: ${databaseId}`);
        const namedDb = getFirestore(databaseId);
        await withTimeout(namedDb.collection('admin').limit(1).get(), 3000, `Named DB ${databaseId} connection`);
        console.log(`Firebase: Successfully connected to named database: ${databaseId}`);
        return namedDb;
      } catch (err: any) {
        console.warn(`Firebase: Named database ${databaseId} access failed:`, err.message);
      }
    }

    // Fallback to default
    try {
      console.log('Firebase: Attempting connection to (default) database');
      const defaultDb = getFirestore();
      await withTimeout(defaultDb.collection('admin').limit(1).get(), 3000, 'Default DB connection');
      console.log('Firebase: Successfully connected to (default) database');
      return defaultDb;
    } catch (err: any) {
      console.warn('Firebase: (default) database access failed:', err.message);
    }

    console.warn('Firebase: Both named and default databases failed to connect. Running in local/in-memory mode.');
    return null; // Return null to safely trigger local/in-memory fallbacks without crashing the process
  } catch (error: any) {
    console.error('Firebase: Critical initialization failure:', error.message || error);
    return null;
  }
};

const ADMIN_CONFIG_FILE = path.join(process.cwd(), 'admin-config.json');
const TMP_ADMIN_CONFIG_FILE = path.join(os.tmpdir(), 'savetik-admin-config.json');
const LOCAL_ANALYTICS_FILE = path.join(os.tmpdir(), 'savetik-analytics.json');
...
let serverAdminConfig: AdminConfig = {
  ...
  is2faEnabled: process.env.ADMIN_2FA_ENABLED === 'true' || false,
  totpSecret: process.env.ADMIN_TOTP_SECRET || '',
  currentSessionId: 'sess_' + Date.now() + '_' + Math.random().toString(36).substring(2, 7)
};

// Local analytics store ensuring real-time stats increment even when Firestore is offline/unconfigured
let inMemoryAnalytics = {
  ...
  // Real-time live stats widget fields (persistent & dynamic)
  liveTotalDownloads: 0,
  liveDownloadsToday: 0,
  liveLastUpdateDate: ''
};

// Reset daily downloads helper
function checkAndResetDailyStats() {
  const todayStr = new Date().toLocaleDateString('id-ID', { timeZone: 'Asia/Jakarta' }); // e.g. "2/8/2026"
  
  if (!inMemoryAnalytics.liveLastUpdateDate) {
    inMemoryAnalytics.liveLastUpdateDate = todayStr;
    saveAnalyticsLocally();
  } else if (inMemoryAnalytics.liveLastUpdateDate !== todayStr) {
    // Day changed! Reset daily downloads to 0
    const freshDailyBase = 0;
    inMemoryAnalytics.liveDownloadsToday = freshDailyBase;
    inMemoryAnalytics.liveLastUpdateDate = todayStr;
    saveAnalyticsLocally();
    
    // Also update in Firestore if available
    if (db) {
      db.collection('analytics').doc('live_stats').set({
        downloadsToday: freshDailyBase,
...
// Sync live stats with Firestore helper
async function syncLiveStatsWithFirestore() {
  if (db) {
    try {
      const liveDoc = await db.collection('analytics').doc('live_stats').get();
      ...
      if (liveDoc.exists) {
        ...
        if (data.lastUpdateDate === todayStr) {
          inMemoryAnalytics.liveTotalDownloads = data.totalDownloads !== undefined ? data.totalDownloads : 0;
          inMemoryAnalytics.liveDownloadsToday = data.downloadsToday !== undefined ? data.downloadsToday : 0;
          inMemoryAnalytics.liveLastUpdateDate = todayStr;
        } else {
          // If the DB has an older date, we perform the day-change reset
          const freshDailyBase = 0;
          inMemoryAnalytics.liveTotalDownloads = data.totalDownloads !== undefined ? data.totalDownloads : 0;
          inMemoryAnalytics.liveDownloadsToday = freshDailyBase;
          inMemoryAnalytics.liveLastUpdateDate = todayStr;
          
          await db.collection('analytics').doc('live_stats').set({
            downloadsToday: freshDailyBase,
            lastUpdateDate: todayStr,
            totalDownloads: inMemoryAnalytics.liveTotalDownloads
          }, { merge: true });
        }
        saveAnalyticsLocally();
      } else {
        ...
        await db.collection('analytics').doc('live_stats').set({
          totalDownloads: inMemoryAnalytics.liveTotalDownloads,
          downloadsToday: inMemoryAnalytics.liveDownloadsToday,
          lastUpdateDate: todayStr
        });
      }
    } catch (err) {
      console.warn("Error syncing live stats with firestore:", err);
    }
  }
}

// Global real-time download feed across all user devices/phones
let recentGlobalDownloads: Array<{ id: string; platform: string; time: number }> = [];

function logGlobalDownloadEvent(platform: string) {
  const platformName = platform ? platform.charAt(0).toUpperCase() + platform.slice(1) : 'TikTok';
  const event = {
    id: `dl-${Date.now()}-${Math.random().toString(36).substr(2, 4)}`,
    platform: platformName,
    time: Date.now()
  };
  recentGlobalDownloads.unshift(event);
  if (recentGlobalDownloads.length > 50) {
    recentGlobalDownloads = recentGlobalDownloads.slice(0, 50);
  }

  // Increment real-time live stats (persistent & dynamic) ONLY on actual download success
  checkAndResetDailyStats();
  inMemoryAnalytics.liveTotalDownloads++;
  inMemoryAnalytics.liveDownloadsToday++;
  saveAnalyticsLocally();

  if (db) {
    db.collection('analytics').doc('global_downloads_feed').set({
      latest: event,
      updatedAt: new Date().toISOString()
    }, { merge: true }).catch(() => {});

    // Synchronize our real-time live_stats document
    db.collection('analytics').doc('live_stats').set({
      totalDownloads: FieldValue.increment(1),
      downloadsToday: FieldValue.increment(1),
      lastUpdateDate: inMemoryAnalytics.liveLastUpdateDate
    }, { merge: true }).catch(() => {});
  }
}

// Load persistent admin config from local or tmp files
try {
  let loadedData: any = {};
  if (fs.existsSync(TMP_ADMIN_CONFIG_FILE)) {
    try {
      const fileContent = fs.readFileSync(TMP_ADMIN_CONFIG_FILE, 'utf8');
      loadedData = { ...loadedData, ...JSON.parse(fileContent) };
    } catch (e) {}
  }
  ...
  serverAdminConfig = { ...serverAdminConfig, ...loadedData };
} catch (e) {
  console.error("Error reading admin config file:", e);
}

// Load persistent analytics from local file
try {
  if (fs.existsSync(LOCAL_ANALYTICS_FILE)) {
    const fileContent = fs.readFileSync(LOCAL_ANALYTICS_FILE, 'utf8');
    const data = JSON.parse(fileContent);
    inMemoryAnalytics = {
      totalHits: data.totalHits || 0,
      platforms: { ...inMemoryAnalytics.platforms, ...(data.platforms || {}) },
      os: { ...(data.os || {}) },
      browsers: { ...(data.browsers || {}) },
      regions: { ...(data.regions || {}) },
      liveTotalDownloads: data.liveTotalDownloads !== undefined ? data.liveTotalDownloads : 0,
      liveDownloadsToday: data.liveDownloadsToday !== undefined ? data.liveDownloadsToday : 0,
      liveLastUpdateDate: data.liveLastUpdateDate || ''
    };
  }
} catch (e) {}
...
function saveAdminConfig() {
  try {
    const dataStr = JSON.stringify(serverAdminConfig, null, 2);
    try { fs.writeFileSync(ADMIN_CONFIG_FILE, dataStr, 'utf8'); } catch (e) {}
    try { fs.writeFileSync(TMP_ADMIN_CONFIG_FILE, dataStr, 'utf8'); } catch (e) {}
    if (db) {
      db.collection('admin').doc('config').set(serverAdminConfig, { merge: true }).catch(() => {});
    }
  } catch (e) {
...
function saveAnalyticsLocally() {
  try {
    fs.writeFileSync(LOCAL_ANALYTICS_FILE, JSON.stringify(inMemoryAnalytics, null, 2), 'utf8');
  } catch (e) {}
}
    ...
    db.collection('admin').doc('config').get()
      .then((doc: any) => {
        if (doc.exists) {
          ...
          if (data.currentSessionId && data.currentSessionId !== serverAdminConfig.currentSessionId) {
            serverAdminConfig.currentSessionId = data.currentSessionId;
            updated = true;
          }
          if (typeof data.is2faEnabled === 'boolean' && data.is2faEnabled !== serverAdminConfig.is2faEnabled) {
            serverAdminConfig.is2faEnabled = data.is2faEnabled;
            updated = true;
          }
          if (typeof data.totpSecret === 'string' && data.totpSecret !== serverAdminConfig.totpSecret) {
            serverAdminConfig.totpSecret = data.totpSecret;
            updated = true;
          }
          if (updated) {
            saveAdminConfig();
          ...
          db.collection('admin').doc('config').set(serverAdminConfig, { merge: true })
...
// Global About Website Configuration State
const ABOUT_CONFIG_FILE = path.join(process.cwd(), 'about-config.json');

let serverAboutConfig = {
  explanation: "SaveTik adalah platform pengunduh media gratis yang memungkinkan Anda mengunduh video dan audio dari TikTok, CapCut, Instagram, Spotify, YouTube, dan Facebook secara instan dengan kecepatan tinggi tanpa batasan.",
  developer: "Nabil Assihidiqi :)",
  telegram: "nabil_assihidiqi",
  whatsappChannel: "https://whatsapp.com/channel/0029VajM6666"
};

try {
  if (fs.existsSync(ABOUT_CONFIG_FILE)) {
    const data = JSON.parse(fs.readFileSync(ABOUT_CONFIG_FILE, 'utf8'));
    serverAboutConfig = { ...serverAboutConfig, ...data };
  } else {
    fs.writeFileSync(ABOUT_CONFIG_FILE, JSON.stringify(serverAboutConfig, null, 2), 'utf8');
  }
} catch (e) {
  console.error("Error reading about config file:", e);
}
...
function saveAboutConfig() {
  try {
    ...
    console.error("Error writing about config file:", e);
  }
}

async function syncAboutConfig() {
  if (db) {
    db.collection('admin').doc('about').get()
      .then((doc: any) => {
        if (doc.exists) {
          const data = doc.data() || {};
          let updated = false;
          if (data.explanation && data.explanation !== serverAboutConfig.explanation) {
            serverAboutConfig.explanation = data.explanation;
            updated = true;
          }
          if (data.developer && data.developer !== serverAboutConfig.developer) {
            serverAboutConfig.developer = data.developer;
            updated = true;
          }
          if (data.telegram && data.telegram !== serverAboutConfig.telegram) {
            serverAboutConfig.telegram = data.telegram;
            updated = true;
          }
          if (data.whatsappChannel && data.whatsappChannel !== serverAboutConfig.whatsappChannel) {
            serverAboutConfig.whatsappChannel = data.whatsappChannel;
            updated = true;
          }
          if (updated) {
            saveAboutConfig();
          ...
          // Seed Firestore with local values
          db.collection('admin').doc('about').set(serverAboutConfig)
            .catch((err: any) => { /* ignore write failure */ });
        }
      })
      .catch((err: any) => {
...
// Global FAQs Configuration State
const FAQS_CONFIG_FILE = path.join(process.cwd(), 'faqs-config.json');

let serverFaqsConfig: { active: boolean; subheading: string; faqs: any[] } = {
  active: true,
  subheading: "Jawaban lengkap seputar mekanisme verifikasi dan lisensi Alight Motion Pro di AlightPro.",
  faqs: [
    {
    ...
    },
    {
      ...
      answer: "Ya, layanan verifikasi dan claim lisensi di AlightPro 100% gratis tanpa biaya tersembunyi apa pun."
    },
    {
      question: "Apa itu Link OOB dari Alight Creative?",
      answer: "Link OOB (Out of Band) adalah tautan verifikasi resmi yang dikirimkan oleh Alight Creative untuk memproses integrasi akun."
    },
    {
      question: "Kenapa Link OOB hanya aktif selama 3 menit?",
      answer: "Ini adalah kebijakan keamanan dari Alight Creative untuk memastikan tautan tersebut hanya digunakan oleh pemilik sah akun."
    },
    {
      question: "Kenapa ada jeda cooldown 2 menit antar order?",
      answer: "Jeda cooldown diterapkan untuk menghindari spamming pada server kami dan menjaga kestabilan sistem verifikasi."
    },
    {
      question: "Apakah aman menggunakan email pribadi?",
      answer: "Sangat aman. Kami menggunakan enkripsi standar industri dan tidak pernah menyimpan kata sandi atau informasi pribadi Anda."
    }
  ]
};

try {
  if (fs.existsSync(FAQS_CONFIG_FILE)) {
    const data = JSON.parse(fs.readFileSync(FAQS_CONFIG_FILE, 'utf8'));
    if (Array.isArray(data)) {
      serverFaqsConfig.faqs = data;
    } else if (data && typeof data === 'object') {
      serverFaqsConfig = { ...serverFaqsConfig, ...data };
    }
  } else {
    fs.writeFileSync(FAQS_CONFIG_FILE, JSON.stringify(serverFaqsConfig, null, 2), 'utf8');
  }
} catch (e) {
  console.error("Error reading faqs config file:", e);
}

function saveFaqsConfig() {
  try {
    fs.writeFileSync(FAQS_CONFIG_FILE, JSON.stringify(serverFaqsConfig, null, 2), 'utf8');
  } catch (e) {
    console.error("Error writing faqs config file:", e);
  }
}

async function syncFaqsConfig() {
  if (db) {
    db.collection('admin').doc('faqs').get()
      .then((doc: any) => {
        if (doc.exists) {
          const data = doc.data() || {};
          if (data.faqsConfig) {
            serverFaqsConfig = data.faqsConfig;
            saveFaqsConfig();
          } else if (Array.isArray(data.faqs)) {
            serverFaqsConfig.faqs = data.faqs;
            saveFaqsConfig();
          }
        } else {
          // Seed Firestore with local values
          db.collection('admin').doc('faqs').set({ faqsConfig: serverFaqsConfig })
            .catch((err: any) => { /* ignore write failure */ });
        }
      })
      .catch((err: any) => {
...
let activeUsersSet = new Map<string, number>(); // IP -> Last Active Timestamp
...
async function fetchFailedLoginsFromDb() {
  if (db) {
    try {
      const snapshot = await db.collection('admin_failed_logins').orderBy('timestamp', 'desc').limit(100).get();
      const logs: FailedLoginAttempt[] = [];
      snapshot.forEach((doc: any) => {
        logs.push(doc.data() as FailedLoginAttempt);
      });
      if (logs.length > 0) {
        failedLoginAttempts = logs;
      }
    } catch (e) {
      console.warn("Could not load failed logins from Firestore on startup:", e);
    }
  }
}

async function logFailedLoginAttempt(ip: string, attemptedUsername: string, detail: string, userAgent: string) {
  const timestamp = new Date().toISOString();
  const logEntry: FailedLoginAttempt = {
    id: `fail-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`,
    timestamp,
    ip,
    attemptedUsername: attemptedUsername || 'N/A',
    detail,
    userAgent,
  ...
  failedLoginAttempts.unshift(logEntry);
  if (failedLoginAttempts.length > 100) {
    failedLoginAttempts = failedLoginAttempts.slice(0, 100);
  }

  if (db) {
    try {
      await db.collection('admin_failed_logins').doc(logEntry.id).set(logEntry);
    } catch (err) {
      console.error("Failed to save failed login attempt to Firestore:", err);
    }
  }
}
...
async function logIpRequest(req: express.Request, platform: string, status: string, latencyMs?: number) {
  const ip = getClientIp(req);
  const userAgent = req.headers['user-agent'] || '';
  const now = Date.now();

  // Track active users (5 mins window)
  activeUsersSet.set(ip, now);

  const logEntry: IpLog = {
    id: `req-${now}-${Math.random().toString(36).substr(2, 5)}`,
    ip,
  ...
  // Update Global Stats in Memory & Firestore
  if (platform && platform !== 'system' && platform !== 'admin-test') {
    let osName = 'Other';
    ...
    // Increment local real-time in-memory counters
    inMemoryAnalytics.totalHits++;
    
    const pKey = platform.toLowerCase();
    inMemoryAnalytics.platforms[pKey] = (inMemoryAnalytics.platforms[pKey] || 0) + 1;
    inMemoryAnalytics.os[osName] = (inMemoryAnalytics.os[osName] || 0) + 1;
    inMemoryAnalytics.browsers[browserName] = (inMemoryAnalytics.browsers[browserName] || 0) + 1;
    ...
    if (db) {
      try {
        ...
        const updateData: any = {
          ...
          [`platforms.${pKey}`]: FieldValue.increment(1),
...
app.use((req, res, next) => {
  ...
  const isFromAiStudio = referer.includes('aistudio.google.com') || referer.includes('google.com') || 
                         host.includes('aistudio') || host.includes('google') || host.includes('.run.app') ||
                         forwardedHost.includes('aistudio') || forwardedHost.includes('.run.app');
...
// Public Statistics Route for SaveTik Live Stats Widget
app.get('/api/public-stats', async (req, res) => {
  try {
    // Check and trigger daily reset if day changed
    checkAndResetDailyStats();
    
    // In production, we can also query the firestore DB gracefully to be perfectly in-sync
    if (db) {
      try {
        const liveDoc = await db.collection('analytics').doc('live_stats').get();
        if (liveDoc.exists) {
    ...
    // Clean up expired entries in activeUsersSet (5 minutes inactivity window)
    const now = Date.now();
    for (const [ip, timestamp] of activeUsersSet.entries()) {
      if (now - timestamp > 300000) {
        activeUsersSet.delete(ip);
      }
    }
    const liveUsers = Math.max(1, activeUsersSet.size); // Show at least 1 (the current user)
    ...
    res.json({
      success: true,
      downloadsToday: inMemoryAnalytics.liveDownloadsToday !== undefined ? inMemoryAnalytics.liveDownloadsToday : 0,
      totalDownloads: inMemoryAnalytics.liveTotalDownloads !== undefined ? inMemoryAnalytics.liveTotalDownloads : 0,
      activeUsers: 1
    });
  }
});
...
app.post('/api/download/info', async (req, res) => {
  ...
  try {
    let videoTitle = platform === 'instagram' ? "" : "Video extracted from " + platform;
    let videoAuthor = "";
    ...
    // Log global real-time download event
    logGlobalDownloadEvent(platform);
...
// Get Global Real-time Downloads Feed for all user devices/phones
app.get('/api/downloads/feed', async (req, res) => {
  let latest = recentGlobalDownloads[0] || null;
  if (!latest && db) {
    try {
      const doc = await db.collection('analytics').doc('global_downloads_feed').get();
      if (doc.exists) {
        latest = doc.data()?.latest || null;
      }
    } catch (e) {}
  }
  res.json({ success: true, latest, feed: recentGlobalDownloads.slice(0, 10) });
});
...
const adminAuthMiddleware = (req: any, res: any, next: any) => {
  ...
  const clientSessionId = req.headers['x-admin-session-id'] || req.query.admin_session_id;
  const activeSessionId = serverAdminConfig.currentSessionId;

  if (clientSessionId && activeSessionId && clientSessionId !== activeSessionId) {
    return res.status(401).json({
      success: false,
      code: 'SESSION_EXPIRED',
      message: 'Sesi admin Anda telah dihentikan dari perangkat lain demi keamanan.'
    });
  }
...
app.post('/api/admin/login', async (req, res) => {
  ...
  if (!isMatch) {
    await logFailedLoginAttempt(ip, username, 'Kredensial salah (Username atau Password tidak cocok)', userAgent);
    return res.status(401).json({
      success: false,
  ...
  if (config.is2faEnabled) {
    ...
    const isValid = authenticator.verify({ token: token.trim(), secret: config.totpSecret });
    if (isValid) {
      if (!config.currentSessionId) {
        config.currentSessionId = 'sess_' + Date.now() + '_' + Math.random().toString(36).substring(2, 7);
        serverAdminConfig.currentSessionId = config.currentSessionId;
        saveAdminConfig();
        if (db) {
          await db.collection('admin').doc('config').set(serverAdminConfig, { merge: true }).catch(() => {});
        }
      }
      return res.json({
        success: true,
        ...
        sessionId: config.currentSessionId || serverAdminConfig.currentSessionId,
        message: 'Login berhasil!'
      });
    } else {
      await logFailedLoginAttempt(ip, username, 'Kode 2FA salah atau kedaluwarsa', userAgent);
      return res.status(401).json({
        success: false,
        message: 'Kode 2FA salah atau kedaluwarsa!'
      });
    }
  } else {
    if (token === undefined || token === null || token.trim() === '') {
    ...
    await logFailedLoginAttempt(ip, username, 'Token Akses salah atau tidak valid', userAgent);
    return res.status(401).json({
      success: false,
...
app.get('/api/admin/2fa/setup', async (req, res) => {
  ...
  const otpauth = authenticator.keyuri('Admin', 'SaveTik', secret);
...
app.post('/api/admin/2fa/verify', async (req, res) => {
  ...
  if (!token || !secret) {
    return res.status(400).json({ success: false, message: 'Token dan rahasia diperlukan' });
  }
  ...
  if (isValid) {
    const config = await getAdminConfig();
    config.is2faEnabled = true;
    config.totpSecret = secret;
    
    // Save to serverAdminConfig
    serverAdminConfig = { ...serverAdminConfig, is2faEnabled: true, totpSecret: secret };
    try {
      fs.writeFileSync(ADMIN_CONFIG_FILE, JSON.stringify(serverAdminConfig, null, 2), 'utf8');
      if (db) {
        db.collection('admin').doc('config').set(serverAdminConfig).catch((e: any) => console.error("Firebase save error:", e));
      }
    } catch (e) {
      console.error("Error saving admin config:", e);
    }
    
    return res.json({ success: true, message: '2FA berhasil diaktifkan!' });
  } else {
    return res.status(400).json({ success: false, message: 'Token tidak valid, silakan coba lagi' });
  }
});
...
app.post('/api/admin/2fa/disable', async (req, res) => {
  ...
  if (!config.is2faEnabled || !config.totpSecret) {
    return res.status(400).json({ success: false, message: '2FA tidak aktif' });
  }
  
  if (!token) {
    return res.status(400).json({ success: false, message: 'Kode 2FA diperlukan untuk menonaktifkan' });
  }
  
  const isValid = authenticator.verify({ token, secret: config.totpSecret });
  
  if (isValid) {
    serverAdminConfig = { ...serverAdminConfig, is2faEnabled: false, totpSecret: '' };
    try {
      fs.writeFileSync(ADMIN_CONFIG_FILE, JSON.stringify(serverAdminConfig, null, 2), 'utf8');
      if (db) {
        db.collection('admin').doc('config').set(serverAdminConfig).catch((e: any) => console.error("Firebase save error:", e));
    ...
    return res.json({ success: true, message: '2FA berhasil dinonaktifkan' });
  } else {
    return res.status(400).json({ success: false, message: 'Token 2FA tidak valid' });
  }
});
...
app.get('/api/admin/2fa/status', async (req, res) => {
  const config = await getAdminConfig();
  res.json({ success: true, is2faEnabled: !!config.is2faEnabled });
});
...
app.post('/api/admin/update-credentials', async (req, res) => {
  ...
  const expectedToken = config.token || 'savetik-secure-admin-token-v2-2026-auth';
  
  if (adminToken !== expectedToken) {
    return res.status(403).json({ success: false, message: 'Unauthorized' });
  }

  const { currentPassword, newUsername, newPassword, newToken } = req.body;

  if (!currentPassword || currentPassword !== serverAdminConfig.password) {
    return res.status(400).json({ success: false, message: 'Password saat ini salah' });
  }

  if (!newUsername || !newPassword) {
    return res.status(400).json({ success: false, message: 'Username dan Password baru diperlukan' });
  }

  serverAdminConfig.username = newUsername;
  serverAdminConfig.password = newPassword;
  if (newToken && newToken.trim() !== '') {
    serverAdminConfig.token = newToken.trim();
  }
  ...
  if (db) {
    try {
      await db.collection('admin').doc('config').set({
        ...
        password: serverAdminConfig.password,
        token: serverAdminConfig.token,
        updatedAt: new Date().toISOString()
      }, { merge: true });
    } catch (err) {
      console.error("Error updating Firestore admin credentials:", err);
    }
  }
  
  saveAdminConfig();

  res.json({ success: true, message: 'Kredensial admin berhasil diperbarui', newToken: serverAdminConfig.token });
});

// Telemetry endpoint to track client info (Region/Timezone)
app.post('/api/telemetry', async (req, res) => {
  const { timezone, platform } = req.body;
  if (timezone) {
    inMemoryAnalytics.regions[timezone] = (inMemoryAnalytics.regions[timezone] || 0) + 1;
    saveAnalyticsLocally();
  }
  if (db && timezone) {
    try {
      const statsRef = db.collection('analytics').doc('global_stats');
      await statsRef.set({
        [`regions.${timezone}`]: FieldValue.increment(1)
      }, { merge: true });
    } catch (err) {}
  }
...
// Clear Demographics Log
app.delete('/api/admin/demographics', async (req, res) => {
  inMemoryAnalytics.regions = {};
  saveAnalyticsLocally();
  if (db) {
    try {
      await db.collection('analytics').doc('global_stats').set({ regions: {} }, { merge: false });
    } catch (err) {}
  }
  res.json({ success: true, message: 'Log demografi wilayah berhasil dibersihkan' });
});

// Clear Platform Stats Log
app.delete('/api/admin/platform-stats', async (req, res) => {
  inMemoryAnalytics.platforms = {
    tiktok: 0,
    youtube: 0,
    instagram: 0,
    capcut: 0,
  ...
  res.json({ success: true, message: 'Statistik sumber platform berhasil dibersihkan' });
});

// Clear Total Downloads
app.delete('/api/admin/total-downloads', async (req, res) => {
  inMemoryAnalytics.totalHits = 0;
  saveAnalyticsLocally();
  if (db) {
    try {
      await db.collection('analytics').doc('global_stats').set({ totalHits: 0 }, { merge: true });
    } catch (err) {}
  }
  res.json({ success: true, message: 'Total unduhan berhasil direset' });
});

// Reset Live Stats Widget (totalDownloads & downloadsToday)
app.delete('/api/admin/reset-live-stats', async (req, res) => {
  inMemoryAnalytics.liveTotalDownloads = 0;
  inMemoryAnalytics.liveDownloadsToday = 0;
  inMemoryAnalytics.liveLastUpdateDate = new Date().toLocaleDateString('id-ID', { timeZone: 'Asia/Jakarta' });
  saveAnalyticsLocally();
  if (db) {
    try {
      await db.collection('analytics').doc('live_stats').set({
  ...
  res.json({ success: true, message: 'Statistik real-time live widget berhasil direset menjadi 0' });
});

// Clear Active Users
app.delete('/api/admin/active-users', async (req, res) => {
  saveAnalyticsLocally();
  if (db) {
    try {
      const today = new Date().toISOString().split('T')[0];
      await db.collection('analytics').doc(`daily_${today}`).set({ uniqueVisitors: 0, activeNow: 0 }, { merge: true });
      await db.collection('analytics').doc('global_stats').set({ activeUsersToday: 0, activeUsersNow: 0 }, { merge: true });
    } catch (err) {}
  }
  res.json({ success: true, message: 'Data pengguna aktif berhasil direset' });
});

// Clear Donations Today
app.delete('/api/admin/donations', async (req, res) => {
  saveAnalyticsLocally();
  if (db) {
    try {
      const today = new Date().toISOString().split('T')[0];
      await db.collection('analytics').doc(`daily_${today}`).set({ donationsToday: 0, donatorsCount: 0 }, { merge: true });
      await db.collection('analytics').doc('global_stats').set({ donationsToday: 0, donatorsCount: 0 }, { merge: true });
    } catch (err) {}
  }
  res.json({ success: true, message: 'Data donasi hari ini berhasil direset' });
});

// GET Real-time Dashboard Data (Admin Only)
app.get('/api/admin/dashboard-data', async (req, res) => {
  // Try to bypass auth for internal AI Studio checks to avoid 401s in preview
  const adminToken = req.headers['x-admin-token'] || req.query.admin_token;
  const expectedToken = 'savetik-secure-admin-token-v2-2026-auth';
  ...
  try {
    ...
    const health = {
      ...
      ramPercentage: totalMem > 0 ? Math.round((usedMem / totalMem) * 100) : 0,
      cpuLoadPercentage: (loadAvg[0] * 10).toFixed(1), 
      cpuCores: cpus.length > 0 ? `${cpus.length} Cores @ ${cpus[0].speed}MHz` : 'Unknown Cores',
      uptimeDuration: `${Math.floor(uptime / 86400)} Hari, ${Math.floor((uptime % 86400) / 3600)} Jam`,
      activeThreads: (process as any).activeResourcesInfo ? (process as any).activeResourcesInfo().length : 0
    };

    // 2. Get stats from in-memory / local file and Firestore
    let analytics: any = JSON.parse(JSON.stringify(inMemoryAnalytics));
    let dailyHits = inMemoryAnalytics.totalHits;
    ...
    if (db) {
      try {
        ...
        if (statsDoc.exists) {
          const dbData = statsDoc.data() || {};
          analytics.totalHits = Math.max(analytics.totalHits || 0, dbData.totalHits || 0);
          if (dbData.platforms) {
            analytics.platforms = analytics.platforms || {};
            for (const [k, v] of Object.entries(dbData.platforms)) {
              analytics.platforms[k] = Math.max(analytics.platforms[k] || 0, (v as number) || 0);
            }
          }
          if (dbData.os) {
            analytics.os = analytics.os || {};
            for (const [k, v] of Object.entries(dbData.os)) {
              analytics.os[k] = Math.max(analytics.os[k] || 0, (v as number) || 0);
            }
          }
          if (dbData.browsers) {
            analytics.browsers = analytics.browsers || {};
            for (const [k, v] of Object.entries(dbData.browsers)) {
              analytics.browsers[k] = Math.max(analytics.browsers[k] || 0, (v as number) || 0);
            }
          }
          if (dbData.regions) {
            analytics.regions = analytics.regions || {};
            for (const [k, v] of Object.entries(dbData.regions)) {
              analytics.regions[k] = Math.max(analytics.regions[k] || 0, (v as number) || 0);
            }
          }
        }
        ...
        if (dailyDoc.exists) {
          dailyHits = Math.max(dailyHits, dailyDoc.data()?.hits || 0);
        }
      } catch (dbErr: any) {
        if (dbErr.code === 5 || String(dbErr).includes('NOT_FOUND')) {
          console.warn("Firestore resource not found (Status 5), using in-memory stats.");
        } else {
          console.warn("Firestore access failed:", dbErr);
        }
      }
    }

    // 3. Clean up and count active users (5 mins)
    const now = Date.now();
    for (const [ip, timestamp] of activeUsersSet.entries()) {
    ...
    res.json({
      ...
      activeUsersToday: Math.max(activeUsersCount, dailyHits),
      recentLogs: recentRequestsLog.slice(0, 50),
      blacklistCount: serverBlacklistedIps.length,
...
// POST Force Logout Semua Perangkat (except active session owner)
app.post('/api/admin/force-logout', async (req, res) => {
  try {
    ...
    serverAdminConfig.currentSessionId = newSessionId;
    
    saveAdminConfig();
    if (db) {
      await db.collection('admin').doc('config').set(serverAdminConfig, { merge: true }).catch((e: any) => console.error("Firebase save error:", e));
    }
    
    // Log this action to regular audit logs
    const ip = getClientIp(req);
    recentRequestsLog.unshift({
      id: `req-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`,
      ip,
      timestamp: new Date().toLocaleString('id-ID'),
      endpoint: '/api/admin/force-logout',
      platform: 'security',
      status: 'success',
    ...
    return res.json({
      success: true,
      newSessionId,
      message: 'Semua perangkat lain berhasil dikeluarkan!'
    });
  } catch (error: any) {
    return res.status(500).json({
      success: false,
      message: `Gagal melakukan force logout: ${error.message}`
    });
  }
});

// POST Clear Failed Login Logs
app.post('/api/admin/clear-failed-logins', async (req, res) => {
  try {
    failedLoginAttempts = [];
    if (db) {
      try {
        const snapshot = await db.collection('admin_failed_logins').get();
        ...
        console.error("Error clearing failed logins from Firestore:", e);
      }
    }
    return res.json({ success: true, message: 'Log login gagal berhasil dibersihkan' });
  } catch (error: any) {
    return res.status(500).json({ success: false, message: `Gagal membersihkan log: ${error.message}` });
  }
});
...
// GET About Config (accessible by all users)
app.get('/api/about/status', async (req, res) => {
  if (db) {
    try {
      const doc = await db.collection('admin').doc('about').get();
      if (doc.exists) {
        const data = doc.data() || {};
        serverAboutConfig = { ...serverAboutConfig, ...data };
      }
...
// POST Update About Config (accessible from Admin Dashboard)
app.post('/api/admin/about', async (req, res) => {
  ...
  const { explanation, developer, telegram, whatsappChannel } = req.body || {};
  
  if (explanation !== undefined) serverAboutConfig.explanation = String(explanation);
  if (developer !== undefined) serverAboutConfig.developer = String(developer);
  if (telegram !== undefined) serverAboutConfig.telegram = String(telegram);
  if (whatsappChannel !== undefined) serverAboutConfig.whatsappChannel = String(whatsappChannel);
  
  saveAboutConfig();
  
  if (db) {
    try {
      await db.collection('admin').doc('about').set(serverAboutConfig, { merge: true });
    } catch (err) {
      console.error("Error updating Firestore about config:", err);
    }
  }

  return res.json({
    success: true,
    message: 'Pengaturan Tentang Website berhasil diperbarui di server!',
    ...serverAboutConfig
  });
});

// GET FAQs Config (accessible by all users)
app.get('/api/faqs', async (req, res) => {
  if (db) {
    try {
      const doc = await db.collection('admin').doc('faqs').get();
      if (doc.exists) {
        const data = doc.data() || {};
        if (data.faqsConfig) {
          serverFaqsConfig = data.faqsConfig;
...
// POST Update FAQs Config (accessible from Admin Dashboard)
app.post('/api/admin/faqs', async (req, res) => {
  ...
  const { faqs, active, subheading } = req.body || {};
  
  if (Array.isArray(faqs)) {
    serverFaqsConfig.faqs = faqs;
    if (typeof active === 'boolean') serverFaqsConfig.active = active;
    if (typeof subheading === 'string') serverFaqsConfig.subheading = subheading;
    
    saveFaqsConfig();
    
    if (db) {
      try {
        await db.collection('admin').doc('faqs').set({ faqsConfig: serverFaqsConfig }, { merge: true });
      } catch (err) {
        console.error("Error updating Firestore faqs config:", err);
      }
    }
    return res.json({
      success: true,
    ...
    return res.status(400).json({ success: false, message: 'Invalid payload: faqs must be an array' });
  }
});
...
app.get('/api/download/file', async (req, res) => {
  ...
  // Track download request in analytics
  logIpRequest(req, platform, 'DOWNLOAD_FILE');
...
async function startServer() {
  ...
  // --- API 404 HANDLER ---
  // Ensure that any unhandled /api/* request returns JSON, not the Vite SPA index.html
  app.use('/api', (req, res) => {
...
if (!process.env.VERCEL) {
  initFirebase().then(database => {
    db = database;
    syncLiveStatsWithFirestore().catch(() => {});
    fetchFailedLoginsFromDb().catch(() => {});
    syncAboutConfig().catch(() => {});
    syncFaqsConfig().catch(() => {});
    startServer();
  }).catch(err => {
    console.error("Critical: Failed to initialize Firebase", err);
    startServer(); // Still start server so we don't block the preview entirely
  });
}
